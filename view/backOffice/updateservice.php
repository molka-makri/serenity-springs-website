...
if (isset($_GET['id'])) {
    $service = $serviceController->getService($_GET['id']);
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Une fois la mise à jour effectuée, rediriger vers la liste
...
<form action="" method="POST">
    <label for="service_type_id">Type de Service</label>
    <select name="service_type_id" required>
        <?php
        $types = $serviceController->listServiceTypes(); // Assurez-vous que vous avez une méthode listServiceTypes dans votre contrôleur
        foreach ($types as $type) {
            $selected = $service['service_type_id'] == $type['service_type_id'] ? 'selected' : '';
            echo "<option value='" . $type['service_type_id'] . "' $selected>" . $type['type_name'] . "</option>";
        }
        ?>
    </select>
    ...
    <input type="text" name="contact" value="<?= htmlspecialchars($service['contact']) ?>" required>

    <label for="photo">Photo (URL)</label>
    <input type="text" name="photo" value="<?= htmlspecialchars($service['photo']) ?>">

    <br><br>
    <!-- Ajouter un bouton de soumission à côté du tableau -->
